// ============================================================================
// api/registrum-report.js — Vercel serverless function
// Přijímá {lux, umbra, weight, day} jednou/den od klienta a zapisuje
// vážený vzorek do chronicon repa (Varianta A, registrum-coenobii-reference.md).
...
const FILE_PATH = 'data/community_register.json';
const BRANCH = 'main';
const MAX_RETRIES = 3;
...
function isValidPayload(body) {
  return !!body
    && typeof body.lux === 'number' && body.lux >= 0 && body.lux <= 100
    && typeof body.umbra === 'number' && body.umbra >= 0 && body.umbra <= 100
    && typeof body.weight === 'number' && body.weight >= 1 && body.weight <= 4
...
async function githubGet(token) {
  const res = await fetch(`https://api.github.com/repos/${REPO}/contents/${FILE_PATH}?ref=${BRANCH}`, {
    headers: {
      ...
      'User-Agent': 'scriptorium-registrum-report',
...
async function githubPut(token, data, sha) {
  ...
  return fetch(`https://api.github.com/repos/${REPO}/contents/${FILE_PATH}`, {
    ...
    body: JSON.stringify({
      message: 'registrum: daily community sample',
      content,
      sha: sha || undefined,
      branch: BRANCH,
    }),
...
module.exports = async function handler(req, res) {
  ...
  const { lux, umbra, weight, day } = req.body;

  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      const { sha, data } = await githubGet(token);

      if (!data[day]) data[day] = { wsum_lux: 0, wsum_umbra: 0, wsum: 0 };
      data[day].wsum_lux += lux * weight;
      data[day].wsum_umbra += umbra * weight;
      data[day].wsum += weight;
      ...
      if (attempt === MAX_RETRIES - 1) {
        console.error('[registrum-report] selhalo po retry:', err.message);
